...
const CIGAR_FIELDS = `
  c.id, COALESCE(c.admin_image_url, c.image_url) as image_url,
...
async function newArrivals(req, res) {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: 'Erreur nouveautés' });
  }
}
...
async function topRated(req, res) {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: 'Erreur top rated' });
  }
}
...
async function bestValue(req, res) {
  try {
    const { rows } = await db.query(`
      ...
      GROUP BY c.id, b.name, co.name
      HAVING COUNT(us.id) >= 2
      ORDER BY qp_ratio DESC
      LIMIT 6
    `);
    res.json(rows);
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Erreur rapport Q/P' });
  }
}
...
async function quickSearch(req, res) {
  ...
  try {
    const { rows } = await db.query(`
      ...
      FROM cigars c
      JOIN brands b ON c.brand_id = b.id
      WHERE b.name ILIKE $1 OR c.name ILIKE $1
